...
class FooterUI extends Component {
    ...
    componentDidUpdate() {  // 当组件更新时
        ...
        // };
        // this.barBallDrag();
        const { musicPlayingState } = this.props;
        const { refs: { audioElement } } = this;
        if (musicPlayingState) {
            audioElement.play()
            this.songRunningFun()
            this.timer = setInterval(() => this.songRunningFun(), 500);
        } else {
            clearInterval(this.timer);
            this.timer = null;
            audioElement.pause()
    ...
    runPauseBut() {  // 点击播放与暂停按钮
        ...
        //     this.refs.runPauseBut.setAttribute("class", "iconfont pause_run iconbofang");
        // }  
        const { musicPlayingState, MUSIC_PLAYING_STATE } = this.props;;
        MUSIC_PLAYING_STATE(!musicPlayingState);
    }

    previousSong() {  // 上一首歌曲的功能  并且把播放状态改为 true
        // let nowMusicIndex = this.props.nowMusicIndex;
        const { nowMusicIndex, MUSIC_PLAYING_STATE, NOW_MUSIC_INDEX, musicList } = this.props;
        let previousSongIndex;
        if (nowMusicIndex === 0) {
            previousSongIndex = this.props.musicList.length - 1;
        } else {
        ...
        // this.props.NOW_MUSIC_INDEX(previousSongIndex);
        // this.props.MUSIC_PLAYING_STATE(true);
        NOW_MUSIC_INDEX(previousSongIndex);
        MUSIC_PLAYING_STATE(true);
        axios.get(musicList[previousSongIndex].lrc).then((res) => {  // 点击上一首时要去获取相应的歌词
    ...
    barBallDrag() {  // 实现小球拖动的功能
        // const barBall = this.refs.barBall;
        // const audioElement = this.refs.audioElement;
        const { refs: { barBall, audioElement } } = this;
        barBall.ontouchstart = function (ev) {
            const touchStartInfo = ev.changedTouches[0];
            const disX = touchStartInfo.pageX - this.offsetLeft;  // 鼠标点击的位置相对于元素本身的位置的
            document.ontouchmove = function (ev) {
    ...
    render() {
        ...
        return (
            <div id="footer">
                ...
                <div className="way">
                    <span className="iconfont how iconloopone"></span>
                    <span className="iconfont iconprevious" onTouchStart={() => this.previousSong()} />
                    {/* <span className="iconfont pause_run iconzanting" onTouchStart={() => { this.runPauseBut() }} ref="runPauseBut"></span> */}
                    <span className={`iconfont pause_run ${musicPlayingState ? 'iconbofang' : 'iconzanting'}`} onTouchStart={() => this.runPauseBut()} ref="runPauseBut" />
                    <span className="iconfont iconnext" onTouchStart={() => this.nextSong()}></span>
                    <span className="iconfont iconziyuan" ></span>
                </div>
...
export default connect(({ musicList, nowMusicIndex, musicPlayingState, nowMusicLyricList, nowMusicLyricIndex }) => 
    ({ musicList, nowMusicIndex, musicPlayingState, nowMusicLyricList, nowMusicLyricIndex }), dispatch => ({ 
        MUSIC_PLAYING_STATE(boolen) {
            dispatch({ type: 'MUSIC_PLAYING_STATE', payload: boolen })
        },
        NOW_MUSIC_INDEX(index) {
